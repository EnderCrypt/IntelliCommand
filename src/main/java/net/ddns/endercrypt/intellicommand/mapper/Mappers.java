...
import net.ddns.endercrypt.intellicommand.mapper.common.FileMapper;
import net.ddns.endercrypt.intellicommand.mapper.common.PathMapper;
 ...
 * @author EnderCrypt
 * 
 *         class that holds all mappers and the class they produce default
 *         mappers exists for all basic primitive datatypes +string
 * 
...
public class Mappers
{
	 ...
	 *             if an internal mapper failed to map the correct object
	 * @throws IntelliCommandException
	 *             if no mapper was available to convert to this specific class
	 * @throws IllegalArgumentException
	 *             if an internal mapper converted to the wrong object class
	 */
	public Object map(String value, Class<?> clazz) throws MapperConversionFailed
	{
		ArgMapper mapper = mappers.get(clazz);
		if (mapper == null) throw new IntelliCommandException("Couldnt find mapper for class " + clazz.getSimpleName());
		
		Object object = mapper.map(value);
		
		if (clazz.isInstance(object) == false) throw new IllegalArgumentException("Mapper for " + clazz + " returned an object of type " + object.getClass());
